...
<script>
    function ViewTabelLaporan() {
        ...
        let tahun = $('#tahun').val();
        if (bulan == "") {
            Swal.fire("Bulan Harus Diisi");
        } else if (tahun == "") {
            Swal.fire("Tahun Harus Diisi");
        } else {
            $.ajax({
                type: "POST",
    ...
    function PrintLaporan() {
        ...
        if (bulan == "") {
            ...
            NewWin = window.open('<?= base_url('Laporan/PrintLaporanBulanan') ?>/' + bulan + '/' + tahun, 'NewWin', 'toolbar=no,width=1100,height=1100,scrollbars=yes');
        }
    }
</script>